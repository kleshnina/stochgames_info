function [coop,pay,s1,freq,Str]=CalcSMEquilibrium_F(qvec,piRound,beta,eps)

% Calculating the strategy distribution and cooperation rates for the population with FULL information
% [coop,pay,s1,freq,Str]=CalcSMEquilibrium_F(qvec,piRound,beta,eps);
...
dim = 8;
Str8=zeros(2^dim,dim); 
for i=1:2^dim, Str8(i,:)=sscanf(dec2bin(i-1,dim), '%1d' )'; end % List of all memory-1 strategies
PayM=zeros(2^dim,2^dim); C=zeros(2^dim,2^dim); S1=zeros(2^dim,2^dim); % Initializing the pairwise payoff matrix, the cooperation matrix, and a matrix for the frequency of being in State 1
for i=1:2^dim
    for j=i:2^dim
...
T=zeros(2^dim,2^dim); % Initializing the transition matrix
for i=1:2^dim
    for j=1:2^dim
        T(i,j)=1/(2^dim-1)*CalcRho(j,i,PayM,N,beta);
...
v=null(T'-eye(2^dim)); freq=v'/sum(v); % Calculating the unique normalized Eigenvector with respect to EV 1
